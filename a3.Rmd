...
#Adding birth year
nobel_winners$birth_year  <- nobel_winners[, birth_year := year(birth_date)]
...
```{r message=FALSE, warning=FALSE}
#How has the average age changed across the years?
a9 <- ggplot(nobel_winners[gender == "female"][, .(count = .N), by = .(gender, prize_year)], aes(x=prize_year, y=count, group = gender)) +
  geom_point() +
  ggtitle("Male Laureates Per Year") +
  ylab("Number of Awards") +
  xlab("Year") +
...
##When did each countries received the most awards?
...
# creating hover 
df2<- df2 %>% mutate(hover = paste0(birth_country, "\n", V1))
...
plot_geo(df2, 
  ...
  add_trace(locations = ~iso3,
            ...
            hoverinfo = "text") %>% 
  layout(geo = map1,
         title = "Countries with most Awards")
```
